...
class _SettingScreen extends State<SettingScreen> {
  final TextEditingController _marksheetnameController =
      TextEditingController(); //マークシートの名前を入力
  final TextEditingController _numberofquestionController =
      TextEditingController(); //問題数を入力
                      ...
                      DropdownButton<String>(
                        ...
                        onChanged: (value) {
                          setState(() {
                            ...
                            final template = templates.firstWhere((template) =>
                                template['name'] == selectedTemplate);

                            // 選択したテンプレートの値を設定する
                            _marksheetnameController.text = selectedTemplate;
                            _numberofquestionController.text =
                                template['questions'].toString();
                            selectedMarkTypes =
                                List<String>.from(template['markTypes']);

                            _isTimeLimitEnabled =
                                template['isTimeLimitEnabled'];
                            ...
                            _isMultipleSelectionAllowed =
                                template['isMultipleSelectionAllowed'];
                          });
                        },
                      ),
                    ],
                    ...
                    onChanged: (value) {
                      ...
                      if (inputNumber != null && inputNumber > 300) {
                        setState(() {
                          ...
                          _numberofquestionController.selection =
                              TextSelection.fromPosition(
                            TextPosition(
                                offset:
                                    _numberofquestionController.text.length),
                          );
                        });
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          TimePickers(
                            ...
                            5,
                            (newValue) => setState(
                                () => _timeLimitHour = newValue), // 値を更新に必要らしい
                          ),
                          Column(
                            children: [
                              Text(""),
                          ...
                          TimePickers(
                            ...
                            59,
                            (newValue) =>
                                setState(() => _timeLimitMinute = newValue),
                          ),
                          Column(
                            children: [
                              Text(""),
                          ...
                          TimePickers(
                            ...
                            59,
                            (newValue) =>
                                setState(() => _timeLimitSecond = newValue),
                          )
                        ],
                      ),
                    ),
  ...
  Widget TimePickers(
      String Time, int _timeLimit, int max, ValueChanged<int> onChanged) {
    return Column(
      children: [
        Text(Time,
            style: TextStyle(
  ...
  void _showMarkSelectDialog(List<String> selectedMarkTypes) {
    ...
    List<String> markCategory_Number = List.generate(
        10, (index) => String.fromCharCode('0'.codeUnitAt(0) + index));
    List<String> markCategory_A2Z = List.generate(
                ...
                builder: (BuildContext context, StateSetter SBsetState) {
                  return Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 10),
                      SelectMarksbyTypes('記号（数学）', markCategory_PlusMinus,
                          selectedMarkTypes, SBsetState),
                      SelectMarksbyTypes('数字', markCategory_Number,
                          selectedMarkTypes, SBsetState),
                      SelectMarksbyTypes('A-Z', markCategory_A2Z,
                          selectedMarkTypes, SBsetState),
                      SelectMarksbyTypes('a-z', markCategory_a2z,
                          selectedMarkTypes, SBsetState),
                      SelectMarksbyTypes('あ-ん', markCategory_Hiragana,
                          selectedMarkTypes, SBsetState),
                      SelectMarksbyTypes('ア-ン', markCategory_Katakana,
                          selectedMarkTypes, SBsetState),
                    ],
                  );
                },
  ...
  ExpansionTile SelectMarksbyTypes(String title, List<String> MarkCategory,
      List<String> selectedMarkTypes, StateSetter SBsetState) {
...
int _getMarkTypeGroup(String mark) {
  if (mark == '+' || mark == '-' || mark == '±') {
    return 0; // 記号
  } else if (mark.codeUnitAt(0) >= '0'.codeUnitAt(0) &&
      mark.codeUnitAt(0) <= '9'.codeUnitAt(0)) {
    return 1; // 数字
  } else if (mark.codeUnitAt(0) >= 'A'.codeUnitAt(0) &&
      mark.codeUnitAt(0) <= 'Z'.codeUnitAt(0)) {
    return 2; // 大文字アルファベット
  } else if (mark.codeUnitAt(0) >= 'a'.codeUnitAt(0) &&
      mark.codeUnitAt(0) <= 'z'.codeUnitAt(0)) {
    return 3; // 小文字アルファベット
  } else if (mark.codeUnitAt(0) >= 'あ'.codeUnitAt(0) &&
      mark.codeUnitAt(0) <= 'ん'.codeUnitAt(0)) {
    return 4; // ひらがな
  } else if (mark.codeUnitAt(0) >= 'ア'.codeUnitAt(0) &&
      mark.codeUnitAt(0) <= 'ン'.codeUnitAt(0)) {
    return 5; // カタカナ
  }
  return 6; // その他
}
